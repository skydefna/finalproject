...
<div class="content mt-3">
    @csrf
        <div class="card-body table-responsive">
            ...
            <table id="tabel-pimpinan" class="table table-bordered">
                <thead>
                    ...
                    @foreach ($pengajuan as $id=>$db)
                        ...
                        <tr>
                            ...
                            <td style="text-align:center; vertical-align: middle;">
                                <form action="{{ route('pengajuan.status.update', $db->id_pengajuan) }}" method="POST">
                                    @csrf
                                    @method('POST')
                                    <select name="status_id" class="form-control font-weight-bold" onchange="this.form.submit()">
                                        @foreach ($status as $statusOption)
                                            <option value="{{ $statusOption->id_status }}"
                                                {{ $db->status->first() && $db->status->first()->id_status == $statusOption->id_status ? 'selected' : '' }}>
                                                {{ ucfirst($statusOption->nama_status) }}
                                            </option>
                                        @endforeach
                                    </select>
                                </form>
            ...
            <script>
                $(document).ready(function() {
                    var table = $('#tabel-pimpinan').DataTable({
                        columnDefs: [
                    ...
                    $('#filter-status').on('change', function () {
                        ...
                        if (selected) {
                            table.column(12).search(selected).draw(); // kolom ke-11 (index 10) = "Status Value"
                        } else {
